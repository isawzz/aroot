...
var z, U, G, UI = {}, Users, Tables, Basepath, Serverdata = {};
...
var Config, Syms, SymKeys, ByGroupSubgroup, KeySets, C52, Cinno, Aristocards, C52Cards;
...
function mAnimate(elem, prop, valist, callback, msDuration = 1000, easing = 'cubic-bezier(1,-0.03,.86,.68)', delay = 0, forwards = 'none') {
	...
	let a = elem.animate(kflist, opts);
...
function mColFlex(dParent, chflex = [1, 5, 1], bgs = [YELLOW, ORANGE, RED]) {
	...
	let res = [];
	for (let i = 0; i < chflex.length; i++) { let d1 = mDiv(dParent, { flex: chflex[i], bg: bgs[i] }); res.push(d1); }
	return res;
...
function mDataTable(reclist, dParent, rowstylefunc, headers, id) {
...
function mInput(dParent, id) {
	let html = `<input type="text" id=${id} class="input" style="max-width:300px">`;
...
function mRemove(elem) {
	if (isString(elem)) elem = mById(elem);
...
function mShield(dParent, styles = { bg: '#00000075' }, id = 'dShield', classnames = null, hideonclick = false) {
...
function mTable(dParent, headers, styles = { mabottom: 0 }, className = 'table') {
	let d = mDiv(dParent);
	let t = mCreate('table');
	mAppend(d, t);
...
function arrRemove(arr, listweg) { arrReplace(arr, listweg, []); }
...
function removeInPlace(arr, el) { arrRemovip(arr, el); }
...
function toWords(s) { return s.split(/(?:,|\s)+/); } //s.split(/\s/); } 
