...
function startWithAssets(){
  ...
  uiTypeCalendar(d1, 6, 2023);

  // let d2 = mDiv('dMain', { w: 800, h: 800, bg: 'orange' })
  // displayCalendar(d2); //mDiv('dMain'));//test0_displayNQuestions();
  //x.setDate(6,2023);
...
function uiTypeCalendar(dParent, month1, year1) {
  ...
  function openModal(date, d, ev) {
    ...
    let d1 = addEditable(d, { w: '100%' }, {
      onEnter: x => {
        ...
        let o = { date, div: d, text: x, title: firstWord(x) };
        saveEvent(o);
        // console.log('o',o)
        // events.push(o);
        // db_save(events)
  ...
  function saveEvent(o){
    ...
    mStyle(inp,{fz:10,cursor:'pointer',
      padding: 3,bg: '#58bae4',fg: 'white',rounding: 5,hmax: 55,
      overflow: 'hidden'});
    inp.setAttribute('readonly',true);
    inp.onclick = ev=>openEvent(ev,o)
    events.push(o);
  ...
  function weiterNachSetDate() {
    ...
    let outerStyles = { patop: 4, weight: 'bold', box: true, 
    ...
    for (let i = 1; i <= paddingDays + daysInMonth; i++) {
      ...
      if (i > paddingDays) {
        ...
        let d=mDiv(daySquare, { align: 'center', bg: rColor(), w: cellWidth - gap, hmin: cellWidth - 28 });
        d.addEventListener('click', ev => openModal(dayString, daySquare.lastChild, ev));

        // //fill w/ existing events
...
function _assemble_code_sorted_orig(list, di, preserveRegions = false) {
	//console.log('...',list[0],di[list[0]]);//var list problem!!!!!
	let text = ''
...
function mClosureUI(dParent) {
	...
	mDiv(dParent, {}, null, '<input type="text" id="inp_project" value="gamesfull"/>')
