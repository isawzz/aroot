...
function _makeNoneGrid(items, options, dGrid) {
	...
	// for (const it of items) { mStyle(lDiv(it), { margin: options.gap / 2, padding: options.padding/2 }); }
	// mStyle(dGrid, { border: '5px solid blue', box: true })
	// let ov = getVerticalOverflow(dGrid);
	if (ov > 0) {
...
function get3ColLine(dParent, idleft, idmiddle, idright, styles = {}) {
	...
	// mStyle(dOuter, { wmin: '100%', vpadding: 4, hpadding: 10, box: true, h: 30 });
