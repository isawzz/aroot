...
function animateColorScale(elem, color = 'green', scale = 1.5, timeoutms = 2000, aniClass = 'scaleInColor') {
	...
	animate(elem, aniClass, timeoutms);
}
function aniInstruction(spoken, v) { if (isdef(spoken)) say(spoken, v); animate(dInstruction, 'onPulse', 500); }
function aniPulse(elem, ms) { animate(elem, 'onPulse', ms); }
//#endregion

//#region onmousedown onmousemove onmouseup drag drop example mit letters und inputs: TODO: generalize!
...
//#region DRAG DROP TEST example mit letters und inputs: TODO: generalize!
var DragElem = null; //is the clone of element from which drag started
var DropZones = []; //all possible drop elements
var DropZoneItem = null;
var DropZoneItems = [];
...
function onDragStartOnLetter1(ev) {
	...
	if (isLetterElement(source)) {
		...
		// von jetzt an un solange DragElem != null ist muss der clone sich mit der maus mitbewegen
		//document.body.ondrag = onDraggingCloneAround;
...
function showSayHint(i) {
	...
	if (spoken) say(spoken); //setTimeout(() => say(spoken), 300+ms);
...
function numberSequenceCorrectionAnimation(stringFunc) {
	...
	let t1 = setTimeout(removeMarkers, 1000);
	let t2 = setTimeout(() => wrong.map(x => { correctWordInput(x); animate(iDiv(x), 'komisch', 1300); }), 1000);
	t4 = setTimeout(() => { if (G.spokenFeedback) say(msg); }, 500);
...
function showCorrectWords(sayit = true) {
	...
	for (const goal of Goal.pics) {
		TOList.correctWords.push(setTimeout(() => {
			let div = mBy(goal.id);
			mClass(div, anim);
			if (speaking) say((G.language == 'E' ? 'the ' : ' ') + goal.correctionPhrase);
		}, to));
		to += ms;
	}
...
function onClickChoice(ev) {
	...
	// if (Goal.choice == Goal.correctChoice) { mStyle(b, { bg: 'green' }); mCheckit(Goal.feedbackUI, 100); }
	// else { mXit(b, 100); }
...
function resetRound() {
	//console.log('...reset round: clearing table and timeouts!!!')
	clearTimeouts();
	clearFleetingMessage();
	clearTable();
	//updateLabelSettings();
...
function say(text, voice = 'random', v = 1, r = .8, p = .9) {
	//console.log('silent',G.silent);
	//if (!G.silent) Speech.say(text, r, p, v, voice);
	Speech.say(text, r, p, v, voice);
}
...
function show_instruction(written, dParent, spoken, { fz, voice } = {}) {
	...
	let v = isdef(voice) ? voice : G.lang == 'E' ? 'random' : G.lang;
	dInstruction.addEventListener('click', () => aniInstruction(spoken, v));
...
function show_title(title){dScore.innerHTML = title;}
