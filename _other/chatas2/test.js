...
function handle_result(result, type) {
	if (type == 'chats') console.log('type', type, '\nresult', result); //return;
	...
	switch (obj.data_type) {
		case "user_info":
			...
			//console.log('NOW!!!');
			let StartIn = 'play'; window['get_' + StartIn](); mBy("radio_" + StartIn).checked = true;
...
// function onClickSidebarWeg(ev) { toggleSidebar(ev, 122, 'Escape'); }
// function onClickSidebarHer(ev) { toggleSidebar(ev, 122, 'Escape'); }
...
function sendHtml(id, filename) {
	...
	html2canvas(document.getElementById(id)).then(function (canvas) {
		...
		ajax.open("POST", "save_url_encoded_image.php", true);
