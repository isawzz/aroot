...
function hexBoard(dParent, rows = 13, cols = 5, wHex = 100) {
	...
	for (let r = 0; r < rows; r++) {
		...
		for (let c = 0; c < curCols; c++) {
			...
			mStyle(dOuter, { position: 'absolute', left: dx + c * wHex, top: r * hline });
			hlist.push(dInner);
...
function hexBoardNO(dParent, rows = 3, cols = 5, sz = 100) {
	...
	for (let r = 0; r < rows; r++) {
		...
		for (let c = 0; c < curCols; c++) {
			...
			mStyle(dOuter, { position: 'absolute', left: dx + c * sz, top: r * hline });
			fields.push(dInner);
		}
		return fields;
	}
...
function hexTest07() {
	...
	for (let r = 0; r < rows; r++) {
		...
		for (let c = 0; c < curCols; c++) {
			...
			mStyle(h, { position: 'absolute', left: dx + c * whexBrut, top: r * hline });
			hlist.push(h);
...
function hexTest03() {
	...
	for (let i = 0; i < 3; i++) {
		let h = mHex03(dParent, whexBrut, 'yellow');
		mStyle(h, { position: 'absolute', left: whexBrut / 2 + i * whexBrut, top: hline });
		hlist.push(h);
	}
}
function hexTest02() {
