...
class SimpleClass7 {
	...
	presentGameState(data) {
		...
		mStyle(dTable, { h: window.innerHeight });
	...
	onDropFreeForm(source, target, ev, clone) {
		...
		mStyle(dField, { left: xField, top: yField }); //,bg:'blue'});
		target.dxy = source.dxy = dxy;
		//es muss noch die haelfte der size diff zwischen perle und field abgezogen werden
		// mStyle(d,{left:x,top:y,w:w,h:h,position:'fixed'});
		// mStyle(d,{left:x+dx,top:y+dy,w:w,h:h,position:'fixed'});
		// mStyle(d,{left:x+dx,top:y+dy,w:w,h:h,position:'fixed'});
	...
	moveCenter(target, source, dx, dy) {
		...
		mStyle(dTarget, { left: newCenter.x - rect.w / 2, top: newCenter.y - rect.h / 2 });
	...
	resetCenter(target) {
		...
		let center = target.center;
		let rect = getRect(dTarget);
		mStyle(dTarget, { left: center.x - rect.w / 2, top: center.y - rect.h / 2 });
		delete target.dxy;
...
function simplestPerlenGame() {
	...
	mStyle(document.body, { opacity: 1 });
...
function createPerle(perle, dParent, sz = 64, wf = 1.3, hf = 0.4, useNewImage = false) {
	...
	if (perle.field != null) { //board perle!
		...
		// 		: 100;
		//sz = Math.max(sz, 50);
		if (isFarbPerle(perle)) mStyle(img, { w: 1, h: 1 });
		else mStyle(img, { w: sz, h: sz });
		mStyle(d, { bg: 'transparent', w: sz, h: sz });
	} else {
		//styling for pool perle
		let d = iDiv(perle);
		mStyle(d, { opacity: 1 - G.settings.dimming / 100 });
		let sz = G.settings.szPoolPerle;
		if (isdef(sz)) {
			mStyle(d.firstChild, { w: sz, h: sz });
		}
