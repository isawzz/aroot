...
function create_table($t){
	...
	$q="INSERT INTO `tables` (`friendly`, `game`, `created`, `modified`, `host`, `players`, `fen`, `options`, `status`) 
		VALUES ('$t->friendly', '$t->game', $created, $modified, '$t->host', '$players', '$fen', '$options', '$t->status')";
...
function delete_game_table($o){
	...
	if (isset($o->req->fen)){
		$fen = $o->req->fen;
		if (!is_string($fen)) $fen = json_encode($fen);	
	}else $fen = 'nix';
	$time = get_now();
	
	$q =  "UPDATE tables SET status='past',fen='$fen',modified=$time WHERE id = $tid"; //ok
	$res=db_write($q);
	//$q="DELETE FROM `players` WHERE tid = $tid";
	//$res=db_write($q);
...
function delete_game_table_BROKEN($tid){
	// $q="DELETE FROM `tables` WHERE id = $tid";
	// $tid = $o->req->tid;
	// if (isset($o->req->fen)){
	// 	$fen = $o->req->fen;
	...
	$q =  "UPDATE tables SET status='past',modified=$time WHERE id = $tid"; //ok
	$res=db_write($q);
	$q="DELETE FROM `players` WHERE tid = $tid";
	$res=db_write($q);
	// $q="DELETE FROM `moves` WHERE tid = $tid";
